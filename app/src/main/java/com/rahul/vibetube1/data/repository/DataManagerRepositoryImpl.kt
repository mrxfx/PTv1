...
import com.rahul.vibetube1.domain.repository.DataManagerRepository
import com.rahul.vibetube1.domain.repository.ImportProgress
import com.rahul.vibetube1.domain.usecase.UpdateUserInterestsUseCase
import com.rahul.vibetube1.workers.ImportWorker
...
class DataManagerRepositoryImpl @Inject constructor(
    ...
    override suspend fun restoreBackup(uri: Uri): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            var backup: ZyvoTubeBackup? = null
                    ...
                    ZipInputStream(inputStream).use { zis ->
                        ...
                        while (entry != null) {
                            ...
                            if (!entry.isDirectory && (name.endsWith("backup.json", ignoreCase = true) || name.endsWith(".json", ignoreCase = true)) && !name.contains("__MACOSX")) {
                                ...
                                backup = gson.fromJson(reader, ZyvoTubeBackup::class.java)
...
data class ZyvoTubeBackup(
    ...
    @SerializedName("preferences") val preferences: ZyvoTubePreferences? = null
)

typealias VibeTubeBackup = ZyvoTubeBackup
typealias PlayTubeBackup = ZyvoTubeBackup

data class ZyvoTubePreferences(
...
typealias VibeTubePreferences = ZyvoTubePreferences
typealias PlayTubePreferences = ZyvoTubePreferences
