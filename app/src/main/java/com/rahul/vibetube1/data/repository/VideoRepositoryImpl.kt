...
import com.rahul.vibetube1.data.network.NewPipeInitializer
...
class VideoRepositoryImpl @Inject constructor(
    ...
    private fun extractVideoStreams(streamInfo: StreamInfo, isLive: Boolean): List<StreamItem> {
        ...
        if (isLive) {
            ...
            streamInfo.videoStreams?.forEach {
                ...
                val size = if (it.bitrate > 0 && durationSeconds > 0) (it.bitrate.toLong() * durationSeconds) / 8L else -1L
                streamsMap[res] = StreamItem(url = it.url ?: "", quality = res, format = it.format?.suffix ?: "mp4", size = size, isAdaptive = false)
            }
            streamInfo.videoOnlyStreams?.forEach {
                val res = it.getResolution() ?: "Unknown"
                val size = if (it.bitrate > 0 && durationSeconds > 0) (it.bitrate.toLong() * durationSeconds) / 8L else -1L
                streamsMap[res] = StreamItem(url = it.url ?: "", quality = res, format = it.format?.suffix ?: "webm", size = size, isAdaptive = true)
            }
        }
    ...
    private fun extractAudioStreams(streamInfo: StreamInfo): List<StreamItem> {
        ...
        return streamInfo.audioStreams?.map {
            val bitrate = if (it.averageBitrate > 0) it.averageBitrate else if (it.bitrate > 0) it.bitrate else 128
            val size = if (bitrate > 0 && durationSeconds > 0) (bitrate.toLong() * 1000L * durationSeconds) / 8L else -1L
            StreamItem(
                url = it.url ?: "",
